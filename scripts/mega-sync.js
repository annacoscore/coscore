/**
 * mega-sync.js
 * Busca o mÃ¡ximo de produtos de cada categoria do Mercado Livre,
 * agrupa variaÃ§Ãµes de cor dentro do mesmo produto, deduplica e
 * salva em scripts/output/catalog.json (preservando existentes).
 *
 * Uso:
 *   node scripts/mega-sync.js
 *
 * VariÃ¡veis de ambiente (ou .env.local):
 *   ML_ACCESS_TOKEN  â€” token de acesso do Mercado Livre
 */
...
const MAX_PER_QUERY = 200;   // mÃ¡ximo de itens por query (4 pÃ¡ginas Ã— 50)
const DELAY_MS      = 150;   // pausa entre requests

// â”€â”€ Categorias de maquiagem que devem ter variaÃ§Ãµes de cor â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Queries por categoria CoScore â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const CATEGORY_QUERIES = {
  // â”€â”€ MAQUIAGEM â”€â”€
  'Batom': [
    'batom maquiagem feminino', 'lipstick batom cor', 'batom cremoso labial matte',
    'batom liquido matte longa duraÃ§Ã£o', 'batom glitter labial', 'batom rosa nude beige',
  ...
  'Base': [
    ...
    'base cushion pele', 'base full coverage', 'base liquida longa duraÃ§Ã£o',
  ...
  'Blush': [
    'blush maquiagem rosinha face', 'blush compacto maÃ§a do rosto', 'blush coral peachy',
  ...
  'Iluminador': [
    'iluminador maquiagem facial glitter', 'highlighter pÃ³ iluminador', 'iluminador liquido',
  ...
  'Contorno/Bronzer': [
    'contorno maquiagem face kit', 'bronzer bronzeador compacto', 'contorno paleta pÃ³',
  ...
  'Delineador': [
    ...
    'delineador loreal maybelline', 'delineador colorido artÃ­stico', 'eyeliner liner gel olhos',
  ],
  'Gloss': [
  ...
  'Fixador de Maquiagem': [
    'fixador maquiagem spray facial', 'setting spray maquiagem longa duraÃ§Ã£o',
  ...
  'Esponjas e PincÃ©is': [
    ...
    'pincel mac sigma sephora', 'esponja base lÃ­quida facial', 'pincel kabuki pÃ³ facial',
  ...
  'Hidratante': [
    'hidratante facial pele seca oleosa', 'creme hidratante rosto nutriÃ§Ã£o', 'moisturizer facial spf',
    ...
    'hidratante corporal pele seca', 'loÃ§Ã£o hidratante corporal neutrogena', 'creme urea hidratante',
    'hidratante facial nÃ£o oleoso', 'gel hidratante facial levinho',
  ],
  'Protetor Solar': [
  ...
  'Limpeza Facial': [
    ...
    'mousse limpeza facial', 'loÃ§Ã£o limpeza facial micelar',
  ],
  'MÃ¡scara Facial': [
  ...
  'Shampoo': [
    ...
    'shampoo liso sedoso cabelo', 'shampoo loiro matizador', 'shampoo nutriÃ§Ã£o intensa',
    ...
    'shampoo reparador danificados', 'shampoo queda prevenÃ§Ã£o',
  ],
  'Condicionador': [
    'condicionador cabelo hidratante nutriÃ§Ã£o', 'condicionador cachos definiÃ§Ã£o',
  ...
  'MÃ¡scara Capilar': [
    'mascara capilar hidratante profunda', 'mascara capilar nutriÃ§Ã£o intensa cachos',
    'creme capilar profundo reparador', 'mascara hidrataÃ§Ã£o intensa cabelo', 'mascara capilar loreal',
    'mascara capilar salon line tresemme', 'mascara capilar wella kerastase',
    'mascara capilar natura avon', 'mascara capilar siÃ ge loreal elvive',
    'tratamento profundo capilar', 'mascara reconstrutora proteica',
  ],
  'Leave-in': [
    'leave-in sem enxague proteÃ§Ã£o cabelo', 'leave-in cachos crespos definiÃ§Ã£o',
  ...
  'Ã“leo Capilar': [
    'oleo capilar argan marroquino', 'oleo capilar nutriÃ§Ã£o brilho', 'serum capilar finalizador',
...
// â”€â”€ Mapeamento de domain_id ML â†’ Categoria CoScore â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// Limpa nome para comparaÃ§Ã£o de duplicatas
...
const JUNK_COLORS = new Set([
  'sem cor', 'Ãºnico', 'Ãºnica', 'unica', 'unico', 'outro', 'outros',
  'multicolor', 'multicor', 'nÃ£o se aplica', 'nao se aplica', '',
  'transparente', 'color', 'Ãºnica cor', 'neutra',
]);
...
// â”€â”€ LÃª catÃ¡logo existente â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function loadCatalog() {
  if (!fs.existsSync(CATALOG_PATH)) return { products: [] };
  return JSON.parse(fs.readFileSync(CATALOG_PATH, 'utf8'));
}

// â”€â”€ Gera ID Ãºnico â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function makeId() {
  return `p${Date.now()}${Math.floor(Math.random() * 9000 + 1000)}`;
}

// â”€â”€ Categoriza produto pela query ou domain_id â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function categorize(prod, targetCategory) {
  // domain_id Ã© mais confiÃ¡vel
...
// â”€â”€ Formata produto ML para catÃ¡logo â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Busca produtos do ML por query â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Enriquece produto com detalhes completos (imagens, etc.) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function enrichProduct(mlId) {
  const data = await get(`https://api.mercadolibre.com/products/${mlId}`);
  return data.error ? null : data;
}

// â”€â”€ Busca cores de um produto (apenas Strategy 1: catalog attributes) â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function getProductColors(mlProd) {
  const colors = [];
  // Cor do prÃ³prio produto catÃ¡logo
...
async function main() {
  console.log('=== Mega Sync CoScore Ã— Mercado Livre ===\n');

  // Carrega catÃ¡logo existente
  const catalog = loadCatalog();
  const existingProducts = catalog.products || [];

  // Ãndices para deduplicaÃ§Ã£o
  ...
  // Para deduplicaÃ§Ã£o intra-sync (nÃ£o adicionar o mesmo mlId duas vezes)
  const seenMlIds   = new Set(existingMlIds);
  const seenParents = new Set();

  // Agrupa por parent_id para unir variaÃ§Ãµes de cor
  const parentGroups = new Map(); // parentId â†’ { product, colors[] }
  ...
  for (const [category, queries] of Object.entries(CATEGORY_QUERIES)) {
    ...
    for (const query of queries) {
      ...
      for (const item of items) {
        ...
        // Se tem parent_id, pode ser variaÃ§Ã£o de cor de produto jÃ¡ visto
        if (parentId && seenParents.has(parentId)) {
          // Adiciona como variaÃ§Ã£o de cor ao produto pai
    ...
    console.log(`  â†’ ${catNew} novos em ${category}`);
  }
  ...
  console.log(`\nâœ… CatÃ¡logo salvo: ${updatedCatalog.total} produtos totais`);
